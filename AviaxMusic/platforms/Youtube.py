...
from Powers.utils.database import is_on_off
from Powers.utils.formatters import time_to_seconds
import os
import glob
import random
import logging
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            try:
                ...
                raise asyncio.TimeoutError(f"API failed: {e}")

            # If we get here, API was successful
            try:
                ...
                raise asyncio.TimeoutError(f"Download failed: {e}")
            except Exception as e:
                print(f"Error occurred while downloading song: {e}")
                raise asyncio.TimeoutError(f"Download failed: {e}")
    except asyncio.TimeoutError:
        # Fall back to cookies-based download
        print("API timeout or failure, falling back to cookies-based download")
        ...
        try:
            ...
            print(f"Cookies-based download also failed: {e}")
            return None
    except Exception as e:
        print(f"Unexpected error in download_song: {e}")
        return None
